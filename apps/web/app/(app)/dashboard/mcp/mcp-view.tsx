 ...
 *
 * It is a separate process on its own port, so none of this is visible from anywhere else
 * in the app: not the endpoint it advertises, not the hostnames it will answer to, not
 * whether it is running. All of it lived in a startup banner readable only over SSH, and a
 * misconfiguration there went unnoticed through an entire deployment.
 *
 * The host list is the reason this page earns its place. The rebinding guard runs after
 * the bearer check, so an unauthenticated probe is refused with 401 whether or not the
 * public hostname is configured — the mistake only surfaces later, as a 403 for every real
 * client at once. Reading the list is the only way to check it, and this is that list.
 */
...
> = {
  authenticating: {
    label: 'answering',
    tone: 'brand',
    note: 'An unauthenticated request is refused with a challenge, which is the healthy response — it is what starts a client’s OAuth flow.',
  },
  unexpected: {
    ...
    <div className="grid gap-6">
      ...
      <LabelFrame title={<Eyebrow as="h3">Accepted Host headers</Eyebrow>}>
        ...
        <p className="mt-2 max-w-[68ch] text-body-sm text-text-muted">
          This cannot be tested by calling the endpoint: the check runs after authentication, so an
          anonymous request is refused the same way whether the hostname is here or not. Reading the
          list is the check.
        </p>
      ...
      <LabelFrame title={<Eyebrow as="h3">Connecting a client</Eyebrow>}>
        <p className="max-w-[68ch] text-body-sm text-text-muted">
          ...
          as the protected resource, the second names the authorization server.
        </p>
        ...
        <p className="mt-4 max-w-[68ch] text-body-sm text-text-muted">
          ...
          land on the same permission check, so there is one authorization decision and not two.
