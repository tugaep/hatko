...
import { Badge, Button, ErrorCard, LabelFrame, SkeletonLine } from '../../../components/ui.tsx';
...
export function IngestionPanel({ onIngested }: { onIngested: () => void }) {
  ...
  return (
    <section aria-label="Ingestion" className="grid gap-4">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h2 className="font-display text-h2 text-text">Ingestion</h2>
        ...
        <div className="flex items-center gap-4">
          ...
          <Button onClick={trigger} loading={running}>
            Run ingestion
          </Button>
        </div>
      </div>
            ...
            <dd
              className={
                label === 'failed' && value > 0
                  ? 'text-mono tabular font-mono text-danger-text'
                  : 'text-mono tabular font-mono text-text'
              }
            >
              {value}
            </dd>
