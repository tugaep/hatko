...
**Ochre is a fill, never text on paper.** `ochre-500` on paper is 2.70:1, so it fails. Ochre appears as a background with dark ink on top, or as `ochre-600` (3.92:1) for icons and 18px+ text only. Never ochre body copy on light.

**Borders that mean something clear 3:1.** Decorative dividers may be faint (`--rule` is 1.24:1 and that's fine, because it separates rather than informs). But any border that defines an interactive target (input outlines, button edges, checkbox frames) uses `--border-interactive`, which clears the 3:1 non-text threshold.
...
Fraunces runs at `opsz` matched to size and `WONK 1` at 30px and above only, because the wonk is character at display size and noise at 20px.
...
Tabular figures are applied contextually via `.tabular`, never globally, because prose should not be tabular.
...
**Borders.** `1px` default, `2px` for selected or active. Never 3px+.
...
**One shadow token exists in the system and only L3 may use it.** A layer floating over arbitrary content cannot establish separation with a border alone, because the content behind it is unknown, so the border may land on a same-value fill. That is a real failure case, not a stylistic preference, so the exception is granted narrowly and nowhere else. If a card wants a shadow, the layout is wrong.
...
Streaming answer text is the one place with continuous motion, and it is text arriving rather than an effect. It must not be accompanied by a cursor blink, a shimmer, or a pulse.
...
Loading replaces the label with a spinner **at the same width**, because buttons must not resize mid-interaction.
...
Label frame. `eyebrow` label, then the figure in `h1` Fraunces `.tabular`, then a `caption` delta in `--text-muted`. No sparkline unless the series has at least 7 points, because a two-point sparkline is a lie.
...
**Error.** L1 card, 1px `--danger`, 20px icon in `--danger-text`, `h4` plain-language cause, `body-sm` detail, retry button. Technical detail (status code, request ID in `mono`) goes in a collapsed `<details>`, available for a bug report rather than shouted at the user.
...
Touch targets: 44×44 minimum below `md`, including citation chips, which means chips get extra vertical padding on touch rather than a smaller hit area.
...
- Gradients on backgrounds, buttons, text, meters or illustrations
