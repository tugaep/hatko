...
The bearer token is a Better Auth session token, the same one the web app uses.
...
Either demo account works, since `search:run` is held by both roles. Tokens expire after
...
| Argument   | Type                   | Notes                                                    |
| ---------- | ---------------------- | -------------------------------------------------------- |
| `query`    | string, 2 to 500 chars | Required. Natural-language questions retrieve best.      |
| `limit`    | integer, 1 to 20       | Optional, defaults to 8. Passages returned after rerank. |
...
as context only, because an abstention the client cannot inspect is one it has to take on
...
not a folder name, so filtering to `guides` will not find `localization-guide.md`.
...
embedding the query and 1 to 2 s in the rerank pass. The per-request `McpServer` and
...
client keeps working until its token expires, which is an hour, or up to seven days if it
